...
pkg_vector <- c("AOI","INLA","NFHL","broom", "caret", "dplyr","exactextractr","ggbreak",
                "gganimate", "ggpmisc", "ggplot2", "gridExtra", "htmltools","htmlwidgets","httr","leaflet",
                "leaflet.extras","lmtest", "plotly", "purrr","raster","readr","reader",
                "rfema","rvest", "scales", "sf", "shiny", "spData","spdep","spatialreg","sphet",
...
floods_with_specific_zones <- st_join(
  ...
  flood_zones_fixed %>% dplyr::select(polygon_id, properties.FLD_ZONE),
  left = TRUE
) %>%
  mutate(
...
norm_vars <- attributions_final_merge %>%
  dplyr::select(total_flood_events, total_flood_zone_area)
