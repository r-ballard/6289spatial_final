...
outputs$COUNT <- as.numeric(outputs$COUNT)
...
missing_in_counties <- outputs %>%
  filter(!(GEOID %in% counties_wkt$GEOID))

missing_in_outputs <- counties_wkt %>%
  filter(!(GEOID %in% outputs$GEOID))
...
outputs <- tibble(
  DFIRM = character(),
  FLD_ZONE = character(),
  geometry = st_sfc(crs = 'NAD83')  # Initialize empty geometry column with CRS
)
...
for (zip_path in zip_files) {
  ...
  # Summarize FLD_ZONE counts
  itertable <- flood_layer %>%
    dplyr::select(DFIRM_ID,FLD_ZONE,geometry)

  # Append to outputs
  outputs <- bind_rows(outputs, itertable)

  # Cleanup
  unlink(temp_dir, recursive = TRUE)
}

write.csv(outputs, file = "data/geom_outputs.csv", row.names = FALSE)
```
